...
const ProductGrid = () => {
  const ref = useRef(null);
  // Adjust useInView: lower amount and add margin for mobile
  ...
  const [filteredProducts, setFilteredProducts] = useState(products);
  // Fallback state for visibility
  const [forceVisible, setForceVisible] = useState(false);

  // Fallback to force visibility after 2 seconds if isInView doesn't trigger
  useEffect(() => {
    const timer = setTimeout(() => {
      if (!isInView) {
        console.log("Forcing visibility due to isInView not triggering");
        ...
        {filteredProducts.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8 mt-10 w-full">
            {filteredProducts.map((product, index) => (
              <motion.div
                key={product.id}
                className="flex flex-col bg-[#FFFFFF] rounded-xl p-4 sm:p-6 shadow-[0_4px_12px_rgba(0,0,0,0.1)] w-full"
                custom={index}
                initial="hidden"
                animate={isInView || forceVisible ? "visible" : "hidden"}
                whileHover="hover"
              ...
              >
                {/* Product Image */}
                <div className="relative w-full aspect-square  mb-4 rounded-lg overflow-hidden p-4">
                  <Image
                    src={product.image || placeholderImage}
                    alt={product.alt || `${product.name} image`}
                    fill
